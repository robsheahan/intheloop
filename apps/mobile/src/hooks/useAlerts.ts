...
export function useAlerts(categorySlug?: string, unseenOnly = false, limit?: number) {
  ...
  return useQuery<AlertHistory[]>({
    ...
    queryFn: async () => {
      ...
      const data = result.data as AlertHistory[];
  ...
  return useQuery<Record<string, number>>({
    ...
    queryFn: async () => {
      ...
      for (const row of result.data || []) {
        const te = row as Record<string, unknown>;
        const tracked = te.tracked_entity as { category: { slug: string } | null } | null;
        const categorySlug = tracked?.category?.slug;
        if (categorySlug) {
